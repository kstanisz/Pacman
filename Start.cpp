...
void initFigures()
{
	...
	float* positionOnScene = new float[3];
	positionOnScene[Dimension(X)] = 6 * 0.5f - 7.25f;
	positionOnScene[Dimension(Y)] = float(0.25 / 2.0);
	positionOnScene[Dimension(Z)] = 22 * 0.5f - 13.75f;

	int* positionOnMap = new int[2];
	positionOnMap[Dimension(X)] = 22;
	positionOnMap[Dimension(Y)] = 6;

	pacMan = new PacMan(positionOnScene, positionOnMap);
}
